...
//animation filler::fillCenterBFS(PNG& img, int x, int y, HSLAPixel centerColor,
                //double tolerance, int frameFreq){
    //HSLAPixel center = *img.getPixel(x,y);
    //customColorPicker a(centerColor, img, tolerance, center);
    //return fill<Queue>(img,x,y,a,tolerance,frameFreq);
//}

//animation filler::fillCenterDFS(PNG& img, int x, int y, HSLAPixel centerColor,
                //double tolerance, int frameFreq){
    //HSLAPixel center = *img.getPixel(x,y);
    //customColorPicker a(centerColor, img, tolerance, center);
    //return fill<Stack>(img,x,y,a,tolerance,frameFreq);
...
{   animation a;
    ...
    *img.getPixel(x,y) = fillColor(x,y);
    ...
    while(!os.isEmpty()){
        pair<int, int> p = os.remove();
        for(int i = 0; i<8; i++){
            if((p.first+pos[i][0]>=0) && (p.second+pos[i][1]>=0) && (p.first+pos[i][0]<=width-1) && (p.second+pos[i][1]<=height-1)
                && (visited[p.first+pos[i][0]][p.second+pos[i][1]] == false)){
                if(img.getPixel(p.first+pos[i][0],p.second+pos[i][1])->dist(center) <= tolerance){
                    os.add(make_pair(p.first+pos[i][0],p.second+pos[i][1]));
                    *img.getPixel(p.first+pos[i][0],p.second+pos[i][1]) = fillColor(p.first+pos[i][0],p.second+pos[i][1]);
                    visited[p.first+pos[i][0]][p.second+pos[i][1]] = true;
                    if(++count % frameFreq == 0){
                        a.addFrame(img);
                    }
                }
